...
## Shortcomings
**127 cells extracted.** NER identifies that "T3" is a staging code but has no way to know which Excel column it belongs to without understanding the surrounding clinical context. There was also no longitudinal linking — each case was treated as an isolated record with no concept of the same patient appearing in multiple proformas. Served primarily as a proof of concept and baseline benchmark.
